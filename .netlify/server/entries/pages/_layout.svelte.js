						...
						return `<ol${spread(
							...
							],
							{
								styles: {
									...
									'--offset':
										typeof offset === 'number' ? `${offset}px` : offset || VIEWPORT_OFFSET,
							...
							$toasts.filter(
								(toast) => (!toast.position && index === 0) || toast.position === position2
							),
							(toast, index2) => {
...
const Layout = create_ssr_component(($$result, $$props, $$bindings, slots) => {
	return `${validate_component(Toaster, 'Toaster').$$render($$result, { richColors: true, position: 'top-center' }, {}, {})} ${slots.default ? slots.default({}) : ``}`;
});
