...
const Icon = create_ssr_component(($$result, $$props, $$bindings, slots) => {
	...
	return `<svg${spread(
		[
			...
			},
			{
				class: escape_attribute_value(
					mergeClasses('lucide-icon', 'lucide', name ? `lucide-${name}` : '', $$props.class)
				)
			}
		],
		{}
		...
		return `${((tag$1) => {
			return tag$1
				? `<${tag}${spread([escape_object(attrs)], {})}>${is_void(tag$1) ? '' : ``}${is_void(tag$1) ? '' : `</${tag$1}>`}`
				: '';
		})(tag)}`;
...
function handleRovingFocus(nextElement) {
	...
	sleep(1).then(() => {
		...
		if (!isHTMLElement$1(currentFocusedElement) || currentFocusedElement === nextElement) return;
...
function createTypeaheadSearch(args = {}) {
	...
	const handleTypeaheadSearch = (key, items) => {
		...
		const currentItemIndex = isHTMLElement$1(currentItem)
...
export {
	...
	removeScroll as i,
	getNextFocusable as j,
	getPreviousFocusable as k,
	addHighlight as l,
	createBitAttrs as m,
	...
	usePopper as u,
	getPositioningUpdater as v,
	createDispatcher as w,
	disabledAttrs as x,
	last as y,
	back as z
};
